...
#
# Outputs:
#   /kaggle/working/audit_final_4core/*.json
#
# Tests:
...
OUT = Path("/kaggle/working/audit_final_4core")
...
SAFE_FEATURES = np.array([i for i in range(79) if i not in (72, 73)], dtype=int)
NO_TIME_FEATURES = np.array([i for i in SAFE_FEATURES if i not in (74, 76)], dtype=int)
...
def load_npz(path, fractions, task):
    ...
    d["feats"][:, :, 72] = 0.0
    d["feats"][:, :, 73] = 0.0
    if np.any(d["feats"][:, :, 72:74] != 0):
        raise RuntimeError("future-length feature neutralization failed")
    if len(np.unique(d["ids"])) != len(d["ids"]):
        raise RuntimeError(f"{task}: duplicate IDs")
    stamp(f"loaded {task}: N={len(d['ids']):,}, feats={d['feats'].shape}; indices 72,73=0 verified")
    return d
...
summary = {
    ...
    "configuration": {
        ...
        "future_length_features_disabled": ["first_breach_old", "n_frac"], "stored_feature_indices": [72, 73],
    },
    "gates": {
        "shuffle_auc_values": shuffle_aucs,
        "shuffle_pass_all_abs_from_half_le_0p02": bool(all(abs(a-0.5) <= 0.02 for a in shuffle_aucs)),
        "future_length_columns_zero_enforced": True,
...
print("Save Version. Send every JSON in /kaggle/working/audit_final_4core", flush=True)
