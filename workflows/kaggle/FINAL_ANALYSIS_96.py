# =====================================================================
#  SESSION B CORRECTED - leakage-free v3 analysis of the scale-up data (up to 19 JSONs)
...
_ANALYSIS_INPUT = _PrePath("/kaggle/working/analysis_input_corrected")
...
if _allocated_cpus < 90:
    raise RuntimeError(
        f"Only {_allocated_cpus} CPUs are allocated; the 96-CPU TPU host is not active."
    )
...
print("XGBoost threads: 96", flush=True)
...
ML_JOBS = int(os.environ.get("V2_ML_JOBS", "96"))
...
def _subset_fracs(d, step, default_fractions=None):
    ...
    Stored features 72 (`first_breach`, old normalization) and 73 (`n_frac`)
    used full recorded length for event-truncated trajectories. Both are set
    identically to zero before every fit. New extraction corrects index 72 and
    retains index 73 only as a zero schema placeholder.  All other columns contain window-sliced quantities.
    """
    ...
    dd["feats"][:, :, 72] = 0.0
    dd["feats"][:, :, 73] = 0.0
    if np.any(dd["feats"][:, :, 72:74] != 0):
        raise RuntimeError("failed to neutralize stored future-length columns 72 and 73")
    return dd
...
os.environ.setdefault("V2_ML_JOBS", "96")  # use all 96 allocated TPU-host CPUs
...
os.makedirs("/kaggle/working/analysis_final", exist_ok=True)
# Remove stale JSONs so a partial rerun cannot mix old and corrected results.
for _old_json in Path("/kaggle/working/analysis_final").glob("*.json"):
    _old_json.unlink()
...
if RUN_MODE in ("TRIPLES", "BOTH"):
    ...
    sys.argv = ["production_analysis.py"] + _args +                ["--out-dir", "/kaggle/working/analysis_final"]
    main()
...
if RUN_MODE in ("ENCOUNTERS", "BOTH"):
    if os.path.exists(_BASE + "/merged_encounters.npz"):
        sys.argv = ["production_analysis.py",
                    ...
                    "--out-dir", "/kaggle/working/analysis_final"]
        main()
...
_all_json = {}
for _jp in sorted(Path("/kaggle/working/analysis_final").glob("*.json")):
    if _jp.name != "report.json":
        with open(_jp) as _fh:
            _all_json[_jp.stem] = json.load(_fh)
_all_json["audit"] = {
    "future_length_features_disabled": ["first_breach_old", "n_frac"],
    "stored_feature_indices": [72, 73],
    "values_used_in_all_models": [0.0, 0.0],
...
with open("/kaggle/working/analysis_final/report.json","w") as _fh:
    json.dump(_all_json,_fh,indent=1)
with open("/kaggle/working/analysis_final/analysis_audit.json","w") as _fh:
    json.dump(_all_json["audit"],_fh,indent=1)
print("\n=== FINAL LEAKAGE-SAFE SESSION B COMPLETE ===", flush=True)
print(sorted(os.listdir("/kaggle/working/analysis_final")), flush=True)
print("IMPORTANT: use only /kaggle/working/analysis_final outputs. Save Version now.")
