...
os.makedirs("/kaggle/working/analysis_final_4core", exist_ok=True)
# Remove stale JSONs so a partial rerun cannot mix old and corrected results.
for _old_json in Path("/kaggle/working/analysis_final_4core").glob("*.json"):
    _old_json.unlink()
...
if RUN_MODE in ("TRIPLES", "BOTH"):
    ...
    sys.argv = ["production_analysis.py"] + _args +                ["--out-dir", "/kaggle/working/analysis_final_4core"]
    main()
...
if RUN_MODE in ("ENCOUNTERS", "BOTH"):
    if os.path.exists(_BASE + "/merged_encounters.npz"):
        sys.argv = ["production_analysis.py",
                    ...
                    "--out-dir", "/kaggle/working/analysis_final_4core"]
        main()
...
_all_json = {}
for _jp in sorted(Path("/kaggle/working/analysis_final_4core").glob("*.json")):
...
with open("/kaggle/working/analysis_final_4core/report.json","w") as _fh:
    json.dump(_all_json,_fh,indent=1)
with open("/kaggle/working/analysis_final_4core/analysis_audit.json","w") as _fh:
    json.dump(_all_json["audit"],_fh,indent=1)
print("\n=== FINAL LEAKAGE-SAFE SESSION B COMPLETE ===", flush=True)
print(sorted(os.listdir("/kaggle/working/analysis_final_4core")), flush=True)
print("IMPORTANT: use only /kaggle/working/analysis_final_4core outputs. Save Version now.")
