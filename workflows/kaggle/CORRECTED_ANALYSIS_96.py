...
os.makedirs("/kaggle/working/analysis_corrected", exist_ok=True)
# Remove stale JSONs so a partial rerun cannot mix old and corrected results.
for _old_json in Path("/kaggle/working/analysis_corrected").glob("*.json"):
    _old_json.unlink()
...
if RUN_MODE in ("TRIPLES", "BOTH"):
    ...
    sys.argv = ["production_analysis.py"] + _args +                ["--out-dir", "/kaggle/working/analysis_corrected"]
    main()
...
if RUN_MODE in ("ENCOUNTERS", "BOTH"):
    if os.path.exists(_BASE + "/merged_encounters.npz"):
        sys.argv = ["production_analysis.py",
                    ...
                    "--out-dir", "/kaggle/working/analysis_corrected"]
        main()
...
_all_json = {}
for _jp in sorted(Path("/kaggle/working/analysis_corrected").glob("*.json")):
...
with open("/kaggle/working/analysis_corrected/report.json","w") as _fh:
    json.dump(_all_json,_fh,indent=1)
with open("/kaggle/working/analysis_corrected/analysis_audit.json","w") as _fh:
    json.dump(_all_json["audit"],_fh,indent=1)
print("\n=== CORRECTED SESSION B COMPLETE ===", flush=True)
print(sorted(os.listdir("/kaggle/working/analysis_corrected")), flush=True)
print("IMPORTANT: use only /kaggle/working/analysis_corrected outputs. Save Version now.")
